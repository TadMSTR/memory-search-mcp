...
- **GitHub private disclosure:** Use the [Security tab](https://github.com/TadMSTR/memory-search-mcp/security/advisories/new) to submit a private advisory.
- **Email:** Send a description to `security.i9v75@8alias.com` with the subject line `[memory-search-mcp] Security Report`.
...
- Path traversal in memory file reads that exposes content outside the memory directory
- Information disclosure through search results returning sensitive memory or session content
- Dependency vulnerabilities with a plausible exploitation path in memory-search-mcp's usage
